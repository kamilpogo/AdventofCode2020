...
{
	static bool CheckPassword(string rule, string password) {
		var range = rule.Split(' ')[0];
		var min = Convert.ToInt32(range.Split('-')[0]);
		var max = Convert.ToInt32(range.Split('-')[1]);
		var character = rule.Split(' ')[1];
		var count = password.Count(f => f == character[0]);
		return count >= min && count <=max;
	}
	
	static bool CheckPassword2(string rule, string password) {
		var range = rule.Split(' ')[0];
		var i1 = Convert.ToInt32(range.Split('-')[0]) - 1;
		var i2 = Convert.ToInt32(range.Split('-')[1]) - 1;
		var character = rule.Split(' ')[1];
		return password[i1] == character[0] ^ password[i2] == character[0];
	}
	
	public static void Main()
	{
		var l = new List<string>(GetInput().Split(new[] { "\r\n", "\r", "\n" }, StringSplitOptions.RemoveEmptyEntries));
		var kvl = l.Select((a) => new KeyValuePair<string, string>(a.Split(new[] {": "}, StringSplitOptions.None)[0], a.Split(new[] {": "}, StringSplitOptions.None)[1])).ToList();
		
		var validPasswords = 0;
		var validPasswords2 = 0;
		foreach(var kv in kvl) {
			if(CheckPassword(kv.Key, kv.Value)) {
				validPasswords++;
			}
			if(CheckPassword2(kv.Key, kv.Value)) {
				validPasswords2++;
			}
		}
		Console.WriteLine("Check 1: " + validPasswords);
		Console.WriteLine("Check 2 : " + validPasswords2);
	}
